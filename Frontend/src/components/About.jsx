...
<section className="py-16 px-4 sm:px-6 md:px-8 lg:px-12 max-w-7xl mx-auto">
  <h2 className="text-3xl sm:text-4xl font-semibold text-center text-red-400 mb-8 sm:mb-12 animate-on-scroll opacity-0 transition-all duration-1000">
    Our Expert Team
  </h2>
    ...
    {[
      {
        ...
        background: "Former Harvard Professor with 15+ years in EdTech",
      },
      {
        name: "Michael Okonjo",
        role: "AI Research Director",
        background: "PhD in Machine Learning with focus on educational applications",
      },
      {
        name: "Sarah Martinez",
        role: "User Experience Lead",
        background: "Specialized in designing accessible learning interfaces",
      },
    ].map((member, index) => (
      <div 
        key={index} 
        className="p-6 bg-gray-800 rounded-lg shadow-lg text-center overflow-hidden relative animate-on-scroll opacity-0"
        style={{ transitionDelay: `${index * 200}ms`, transform: 'translateY(20px)' }}
      >
        {/* Circle Initials */}
        <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full bg-gradient-to-br from-red-400 to-red-600 mb-4 flex items-center justify-center text-lg sm:text-2xl font-bold">
        ...
        {/* Role */}
        <p className="text-red-400 mb-2 text-sm sm:text-base">{member.role}</p>
        
        {/* Background */}
        <p className="text-gray-400 text-xs sm:text-sm">{member.background}</p>
      </div>
    ))}
  </div>
</section>
